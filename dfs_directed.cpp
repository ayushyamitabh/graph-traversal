	...
	while (!Q.isEmpty()) {
		...
		for (int w = 1; w <= n; ++w)
			...
			if (isConnected(v, w) && !explored[w]) {
				cout << v << '\t' << w << "<<== Connected and Not Explored" << endl;
			...
			else if (isConnected(v, w) && explored[w]) {
				cout << v << '\t' << w << "<<== Connected but Already Explored" << endl;
			}
	}
	cout << endl;
	delete[] explored;
...
int main() {
	...
	int _vertexFrom, _vertexTo, _weight;
	while (!inFile.eof()) {
		inFile >> _vertexFrom >> _vertexTo >> _weight;
		g.addEdge(_vertexFrom, _vertexTo);
	}
	inFile.close();

	/** Explores all vertices findable from vertex 0 */
	g.BFS(0);
	return 0;
}